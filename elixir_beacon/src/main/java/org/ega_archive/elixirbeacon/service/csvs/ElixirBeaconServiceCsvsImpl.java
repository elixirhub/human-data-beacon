...
public class ElixirBeaconServiceCsvsImpl implements ElixirBeaconService {
	...
	public Beacon listDatasets(CommonQuery commonQuery, String referenceGenome) {
		...
		QueryResponse<DiseaseGroup> diseaseGroupQueryResponse = parseResponse.parseCsvsResponse(url,
				DiseaseGroup.class);
		...
		for (DiseaseGroup disease : diseaseGroupQueryResponse.getResult()) {
			datasets.add(Dataset.builder().id(String.valueOf(disease.getGroupId())).name(disease.getName())
					.assemblyId("GRCh37").sampleCount((long) disease.getSamples())
					.variantCount((long) disease.getVariants()).build());
		}
		beacon.setDatasets(datasets);

		url = "http://csvs.clinbioinfosspa.es:8080/csvs/rest/files/samples";
		QueryResponse<Integer> integerQueryResponse = parseResponse.parseCsvsResponse(url, Integer.class);
	...
	@Override
	public Object queryBeacon(List<String> datasetStableIds, String variantType, String alternateBases,
			String referenceBases, String chromosome, Integer start, Integer startMin, Integer startMax, Integer end,
			Integer endMin, Integer endMax, String referenceGenome, String includeDatasetResponses,
			List<String> filters) {

		if (StringUtils.isNotBlank(referenceBases) && StringUtils.isNotBlank(chromosome) && start != null
				&& StringUtils.isBlank(variantType) && startMin == null && startMax == null && endMin == null
				&& endMax == null) {
			...
			if (end == null && StringUtils.isNotBlank(alternateBases)) {
				...
				return genomicQuery.queryBeaconGenomicRegion(datasetStableIds, referenceBases, chromosome, start, end,
	...
	public Object queryBeacon(BeaconRequest request) {
		return queryBeacon(request.getDatasetIds(), request.getVariantType(), request.getAlternateBases(),
				request.getReferenceBases(), request.getReferenceName(), request.getStart(), request.getStartMin(),
				request.getStartMax(), request.getEnd(), request.getEndMin(), request.getEndMax(),
				request.getAssemblyId(), request.getIncludeDatasetResponses(), request.getFilters());
	}
}
